...
def crawl_list_in_info(data_list_p,links,columns):
    for link in links:
        ...
        tbList = soup.find('article',{'class':'artReadCoInfo'}).find('dl',{'class':'tbList'})
        tbRow = soup.find('article',{'class':'artReadWork'})
        if tbRow: tbRow = tbRow.find('div',{'class':'tbRow'})
        ...
        if tbRow:
            tbRow = tbRow.find('dl').find('dd')
            data_list = list_func_append(tbRow,columns,data_list)
...
def main():
    csv_filename = 'company_work_info.csv'
    # 데이터를 저장할 리스트 초기화
    url = 'https://www.jobkorea.co.kr/Search/?duty=1000235&tabType=recruit&Page_No='    # 크롤링할 리스트 주소
    columns=['회사명','산업','사원수','설립','기업형태','매출액','연금·보험','휴무·휴가·행사','사내시설']           # 크롤링해서 넣을 리스트
